...
export const AppProvider = ({ children }) => {
  // Premium state — false by default, only true after confirmed payment
  const [isPremium, setIsPremium] = useState(() => {
    const stored = localStorage.getItem('isPremium');
    const grantedAt = localStorage.getItem('premiumGrantedAt');
    // If isPremium is true but there's no payment timestamp,
    // it was set by the old dev default=true bug — reset it
    if (stored === 'true' && !grantedAt) {
  ...
  useEffect(() => {
    ...
    if (params.get('payment') === 'success') {
      // Activate premium and record payment timestamp
      setIsPremium(true);
